...
// Live (still-counted) June canonical-ish outcomes: target/stop/flat only.
const rows = (await db.execute({
  sql: `SELECT o.signal_id, o.asset_id, o.direction, o.outcome, o.realized_pct,
               o.generated_at, o.catalyst_subtype, n.title
...
// Group by asset+direction+normalized-headline; flag any group with >1 row.
const norm = (t) => String(t ?? "")
  .toLowerCase()
  .replace(/^\[@[^\]]+\]/,"")          // drop [@source] prefix
  .replace(/[⚡️🚨🔥]/g,"")
  .replace(/[^a-z0-9 ]/g," ")
  .replace(/\s+/g," ").trim()
  .slice(0, 50);                        // first ~50 chars of the gist
...
for (const [key, rs] of groups) {
  ...
  // Only interesting if the headline gist is non-empty.
  if (key.split("|")[2].length < 8) continue;
  clusters++;
  console.log(`\nDUPLICATE CLUSTER (${rs.length}) — ${rs[0].asset_id} ${rs[0].direction}`);
  for (const r of rs) {
    console.log(`  ${ds(r.generated_at)} ${r.outcome.padEnd(10)} ${String(r.realized_pct).padStart(7)}%  ${String(r.signal_id).slice(0,12)}  ${String(r.title??"").slice(0,64)}`);
  }
}
if (clusters === 0) console.log("No remaining cross-day duplicate clusters.");
process.exit(0);
