...
// The two consecutive-day BitMNR/$300M-preferred tok-eth LONG dupes.
const candidates = (await db.execute({
  sql: `SELECT o.signal_id, o.asset_id, o.direction, o.outcome, o.realized_pct,
        ...
        WHERE o.asset_id = 'tok-eth' AND o.direction = 'long'
...
const dupeIds = candidates
  .filter((r) => String(r.title ?? "").includes("BitMNR") || String(r.title ?? "").includes("$300M") || String(r.title ?? "").includes("preferred stock"))
  .map((r) => String(r.signal_id));

console.log(`\nDismissing ${dupeIds.length} duplicate(s):`, dupeIds);
for (const id of dupeIds) {
  await db.execute({
    sql: `UPDATE signal_outcomes SET outcome='dismissed', outcome_at=?, notes=COALESCE(notes,'')||' [dismissed: duplicate BitMNR/$300M catalyst]' WHERE signal_id=?`,
    args: [Date.now(), id],
  });
}
console.log("DONE");
